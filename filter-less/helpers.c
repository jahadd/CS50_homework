...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            copy[i][j].rgbtRed = (int) (totalRed / (float) count + 0.5);
            copy[i][j].rgbtGreen = (int) (totalGreen / (float) count + 0.5);
            copy[i][j].rgbtBlue = (int) (totalBlue / (float) count + 0.5);
        }
    }

    // Copiar resultados a la imagen original
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
